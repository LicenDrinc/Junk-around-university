...
robot = DriveBase(left_motor, right_motor, wheel_diameter=62.1, axle_track=206)
left_sensor = ColorSensor(Port.S2)
right_sensor = ColorSensor(Port.S3)
left_S_sensor = ColorSensor(Port.S4)
right_S_sensor = ColorSensor(Port.S1)
hand_motor = Motor(Port.A)
gate_motor = Motor(Port.D, positive_direction=Direction.COUNTERCLOCKWISE)#gears=[12,28,20]
hand_motor.control.limits(1500,5000,1000)
gate_motor.control.limits(1500,5000,1000)
...
xbox = [None,None,None,None] # left => right
...
def xbox_scan(xboxID,LR=False):
    ...
    while i<11:
        ...
        # angle=robot.distance()
        if LR ==True:
            h,s,v=rgb_to_hsv_tuple(right_S_sensor.rgb())
            if 210<h<240:
                c = Color.BLUE
            elif 100 < h < 121:
                c = Color.GREEN
            ...
            h,s,v=rgb_to_hsv_tuple(left_S_sensor.rgb())
            if 215<h<240:
                c = Color.BLUE
            elif 119 < h < 151:
                c = Color.GREEN
    ...
    print(archivecolors)
    c= max(dcolors,key=dcolors.get) if len(dcolors)>0 else None
    print('samotsvet in',xboxID, 'color is', c )
    global xbox
    xbox[xboxID] = None if c not in [Color.BLUE, Color.GREEN] else c

# watch_marking_block_moving(0, start=0, end=100)

def indicator_scan(indicatorID,LR=False):
    #returns green, white or none
    print('marking')
    dcolors={}
    archivecolors=[]
    ...
    global indicator
    indicator[indicatorID] = None if c not in [Color.GREEN,Color.BLUE] else c
...
# a=[]
# while True:
#     print('left | ',rgb_to_hsv_tuple(left_S_sensor.rgb()))
#     print('right | ',rgb_to_hsv_tuple(right_S_sensor.rgb()))
#     # mountain_scan(0)
#     wait(1000)
#     a.append(left_S_sensor.ambient())
#     print(len(a))
# print(len(a))
...
# gate_motor.run_until_stalled(-1500,Stop.BRAKE,50)
# gate_motor.reset_angle(0)
# hand_motor.run_until_stalled(-1500,Stop.BRAKE,50)
# hand_motor.reset_angle(0)
...
# indicator_scan(0)
# indicator_scan(0,True)



# BLUE  | int
# left  | 210-235 , 82-100 , 4-36
# right | 180-235 , 60-100 , 1-12

# GREEN | int
# left  | 110-181 , 79-100 , 1-16
# right | 90-130 , 65-100 , 0-8


# BLUE  | return
# left  | 215-240 , 82-100 , 3-30
# right | 210-240 , 80-100 , 0-12

# GREEN | return
# left  | 119-151 , 74-100 , 1-18
# right | 100-121 , 74-100 , 0-10