...
robot = DriveBase(left_motor, right_motor, wheel_diameter=63.2, axle_track=151.5)
left_sensor = ColorSensor(Port.S2)
right_sensor = ColorSensor(Port.S3)
side_sensor = left_sensor 
front_sensor = ColorSensor(Port.S4)
hand_sensor = front_sensor 
...
gate_motor = Motor(Port.D, positive_direction=Direction.COUNTERCLOCKWISE)#gears=[12,28,20]
...
def gyropid_to_motor(speed=50,p=0.3,i=0.0005,d=5,distance=100,heading=180):
    integral = errold = 0
    robot.reset()
    # right_motor.reset_angle(0)
    while robot.distance()<distance:
        curhead=gyro_sensor.angle()
        prop = (heading-curhead)*p
        integral = integral + (heading-curhead)*i
        diff = (heading-curhead-errold)*d
        errold = heading-curhead
...
def gyroturn(speed=50, heading=100):
    curhead=gyro_sensor.angle()
    if curhead<heading:
        robot.drive(0,speed)
...
def pid_left_to_rightREDX(speed=35,p=0.3,i=0.0005,d=5,stoplevel=40, bw=60):
    integral = errold = 0
    ls = left_sensor.reflection()
    rs = bw
    while 330>rgb_to_hsv_tuple(right_sensor.rgb())[0]>20:
        ls = left_sensor.reflection()
        rs = bw
        prop = (ls-rs)*p
...
mountain = [Color.BROWN, Color.BROWN, Color.BROWN, Color.BROWN, Color.BROWN, Color.BROWN, Color.BROWN, Color.BROWN]
indicator = [Color.BLUE,Color.BLUE,Color.BLUE,Color.BLUE]
print(hospital)
handsClosed=False
...
def mountain_scan(mountainID):
    ...
    print(archivecolors)
    c= max(dcolors,key=dcolors.get) if len(dcolors)>0 else None
    print('samotsvet in',mountainID, 'color is', c )
    global mountain
    mountain[mountainID] = None if c not in [Color.BLUE, Color.GREEN,Color.RED,Color.PURPLE] else c

# watch_marking_block_moving(0, start=0, end=100)

def indicator_scan(indicatorID):
    #returns green, white or none
    print('marking')
    dcolors={}
    archivecolors=[]
    ...
    print(archivecolors)
    c= max(dcolors,key=dcolors.get) if len(dcolors)>0 else None
    print('samotsvet in',indicatorID, 'color is', c )
    global indicator
    indicator[indicatorID] = None if c not in [Color.GREEN,Color.RED] else c
...
# a=[];
# while True:
#     print(rgb_to_hsv_tuple(front_sensor.rgb()))
#     wait(1000)
#     a.append(front_sensor.ambient())
#     print(len(a))
# print(len(a))
...
# robot.turn(-360)
# pid_right_out_to_motor(speed=2000,p=0.3,i=0.0005,d=5,distance=600,bw=55)
...
def Dam_opening():
    if mountain[0]==Color.BLUE or mountain[1]==Color.BLUE or mountain[2]==Color.BLUE or mountain[3]==Color.BLUE:
        # hand_motor.run_target(1500,90)
        pid_to_motor(speed=200,p=0.3,i=0.0001,d=5,distance=100)
        pid_to_X(speed=200,p=0.3,i=0.0001,d=5,stoplevel=40)
        rstop()
        robot.straight(90)
        gyroturn(speed=150,heading=180)
        rstop()
        t = _thread.start_new_thread(hand_motor.run_until_stalled,(-1500,Stop.BRAKE,50))
        # gate_motor.run_target(1500,180*12)
        gate_motor.run_target(1500,180*23)
        robot.straight(-250)
        gyropid_to_motor(speed=100,p=0.3,i=0.001,d=10,distance=250,heading=180)
        gyroturn(speed=150,heading=90)
        rstop()
        pid_to_X(speed=200,p=0.3,i=0.0001,d=5,stoplevel=40)
        rstop()
        robot.straight(90)
    else:
        pid_to_motor(speed=200,p=0.3,i=0.0001,d=5,distance=550)
        pid_to_X(speed=200,p=0.3,i=0.0001,d=5,stoplevel=40)
        rstop()
        robot.straight(90)

    gate_motor.run_target(1500,180*0)
    robot.turn(180)
    robot.straight(-145)
    indicator_scan(0)
    if indicator[0]==Color.GREEN:
        robot.straight(-20)
        gyropid_to_motor(speed=100,p=0.3,i=0.001,d=10,distance=165,heading=270)
        rstop()
        # robot.turn(180)
        # pid_to_X(speed=200,p=0.3,i=0.0001,d=5,stoplevel=40)
    else:
        gyropid_to_motor(speed=100,p=0.3,i=0.001,d=10,distance=145,heading=270)
        rstop()
        # robot.turn(180)
        # pid_to_X(speed=200,p=0.3,i=0.0001,d=5,stoplevel=40)
    gyroturn(speed=100,heading=180)
    rstop()
    pid_to_motor(speed=100,p=0.3,i=0.0001,d=5,distance=100)
    rstop()
    robot.straight(-110)
    t = _thread.start_new_thread(hand_motor.run_until_stalled,(-1500,Stop.BRAKE,50))
    gate_motor.run_target(1500,180*12)
    ...
    print(front_sensor.reflection())
    if front_sensor.reflection()>30:
        ev3.speaker.beep()
        robot.straight(60)
        gate_motor.run_target(1500,180*0)
        robot.straight(-75)
        hand_motor.run_until_stalled(1500,Stop.BRAKE,50)
        hand_motor.run_target(1500,93)
        gate_motor.run_target(1500,180*13)
    else:
        robot.straight(60)
        hand_motor.run_until_stalled(1500,Stop.BRAKE,50)
        robot.straight(-55)
        hand_motor.run_target(1500,80)
        gate_motor.run_target(1500,180*7)
        gate_motor.run_target(1500,180*11)

    robot.straight(90)
    gate_motor.run_target(1500,0)
    gyroturn(speed=150,heading=270)
    rstop()
    pid_to_X(speed=150,p=0.3,i=0.0001,d=5,stoplevel=40)
    rstop()
    robot.straight(200)
    robot.turn(90)
    to_wall(distance=450, speed=200, time_limit=3000)
    rstop()
    wait(2000)
    ...
    gyroturn(speed=150,heading=180)
    rstop()
    gyropid_to_motor(speed=300,p=0.3,i=0.001,d=15,distance=540,heading=180)
    rstop()
    gyroturn(speed=150,heading=90)
    rstop()
    robot.straight(100)
    ...
    robot.straight(-145)
    indicator_scan(1)
    if indicator[1]==Color.GREEN:
        robot.straight(-20)
        gyropid_to_motor(speed=100,p=0.3,i=0.001,d=10,distance=185,heading=270)
        rstop()
        pid_to_X(speed=200,p=0.3,i=0.0001,d=5,stoplevel=40)
    else:
        gyropid_to_motor(speed=100,p=0.3,i=0.001,d=10,distance=165,heading=270)
...
def collection_of_gems(Red=True,Blue=True,continuation=True,cell=0,blind=True):
    if mountain[cell]==Color.BLUE:
        # gate_motor.run_target(1500,180*2)
        # robot.reset()
        # robot.drive(600,140)
        # while robot.angle()<83: pass # 171
        # rstop()
        # robot.straight(120)
        # hand_motor.run_until_stalled(-1500,Stop.BRAKE,50)
        # robot.reset()
        # robot.drive(600,-140)
        # while robot.angle()>-40: pass # 171
        # rstop()
        # robot.reset()
        # robot.drive(-600,-140)
        # while robot.angle()>-40: pass # 171
        ...
        # robot.turn(35)
        # gate_motor.run_target(1500,180*11)
        # pid_to_motor(speed=200,p=0.6,i=0.0005,d=20,distance=100)
        # pid_to_X(speed=200,p=0.3,i=0.0005,d=5,stoplevel=40)
        # robot.straight(-300)
        robot.straight(95)
        robot.reset()
        robot.drive(-600,140)
        while robot.angle()<38: pass # 171
        rstop()
        robot.reset()
        robot.drive(600,140)
        while robot.angle()<38: pass # 171
        ...
        if continuation == True:
            gyropid_to_motor(speed=100,p=0.3,i=0.001,d=10,distance=350,heading=180)
            rstop()
            # pid_to_motor(speed=200,p=0.2,i=0.0001,d=10,distance=200)
            # robot.turn(-90)
            gyroturn(speed=150, heading=90)
            rstop()
            if blind==False and cell!=2:
                robot.reset()
                robot.drive(-150,0)
                while right_sensor.reflection()<45: pass
                ...
                robot.straight(-7)
                hand_motor.run_until_stalled(1500,Stop.HOLD,50)
    elif mountain[cell]==Color.GREEN and Blue==True:
        robot.straight(30)
        gyroturn(speed=150, heading=40)
        rstop()
        # robot.turn(-35)
        robot.straight(-80)
        hand_motor.run_target(1500,130)
        robot.straight(80)
        gyroturn(speed=150, heading=180)
        rstop()
        # robot.turn(35+90)
        ...
        if continuation == True:
            robot.straight(50)
            # pid_to_motor(speed=200,p=0.2,i=0.0001,d=10,distance=180)
            gyropid_to_motor(speed=100,p=0.3,i=0.001,d=10,distance=180,heading=180)
            rstop()
            # robot.turn(-90)
            gyroturn(speed=150, heading=90)
            rstop()
            if blind==False and cell!=2:
                ...
                robot.straight(-7)
                hand_motor.run_until_stalled(1500,Stop.HOLD,50)
    elif mountain[cell]==Color.RED and Red==True:
        robot.straight(60)
        # robot.turn(-135)
        gyroturn(speed=150, heading=-45)
        rstop()
        hand_motor.run_until_stalled(-1500,Stop.BRAKE,50)
        robot.straight(40)
        # robot.turn(-135)
        ...
        if continuation == True:
            # robot.straight(50)
            # pid_to_motor(speed=200,p=0.2,i=0.0001,d=10,distance=212)
            gyropid_to_motor(speed=100,p=0.3,i=0.001,d=10,distance=250,heading=180)
            rstop()
            # robot.turn(-90)
            gyroturn(speed=150, heading=90)
            ...
            if blind==False and cell!=2:
                ...
                robot.straight(-7)
                hand_motor.run_until_stalled(1500,Stop.HOLD,50)
    elif mountain[cell]==Color.RED and Red==False:
        robot.straight(60)
        robot.turn(135)
        hand_motor.run_until_stalled(-1500,Stop.BRAKE,50)
        robot.straight(40)
        ...
        if continuation == True:
            # robot.straight(50)
            # pid_to_motor(speed=200,p=0.2,i=0.0001,d=10,distance=145)
            gyropid_to_motor(speed=100,p=0.3,i=0.001,d=10,distance=190,heading=180)
            rstop()
            # robot.turn(-90)
            gyroturn(speed=150, heading=90)
            rstop()
            if blind==False and cell!=2:
                ...
                robot.straight(-7)
                hand_motor.run_until_stalled(1500,Stop.HOLD,50)
    elif mountain[cell]==Color.PURPLE:
        robot.straight(60)
        hand_motor.run_until_stalled(-1500,Stop.BRAKE,50)
        robot.straight(15)
        # robot.turn(90)
        ...
        if continuation == True:
            ...
            # rstop()
            # robot.straight(50)
            gyropid_to_motor(speed=100,p=0.3,i=0.001,d=10,distance=210,heading=180)
            rstop()
            # pid_to_motor(speed=200,p=0.2,i=0.0001,d=10,distance=145)
            # robot.turn(-90)
            gyroturn(speed=150, heading=90)
            ...
            if blind==False and cell!=2:
                ...
                robot.straight(-7)
                hand_motor.run_until_stalled(1500,Stop.HOLD,50)
    elif mountain[cell]==Color.GREEN and Blue==False:
        robot.straight(30)
        gyroturn(speed=150, heading=115)
        rstop()
        # robot.turn(35)
        robot.straight(-80)
        hand_motor.run_target(1500,130)
    ...
    # if continuation==False and mountain[cell]==Color.RED and cell!=7:
    #     # robot.turn(180)
    #     to_wall(distance=200, speed=200, time_limit=1500)
    #     rstop()
    #     wait(2000)
    #     gyro_sensor.reset_angle(0)
    #     wait(1000)
    ...
    #     gyroturn(speed=150, heading=180)
    #     rstop()
    # elif continuation==False and mountain[cell]==Color.GREEN and cell!=7:
    #     # robot.turn(180)
    #     to_wall(distance=200, speed=200, time_limit=1000)
    #     rstop()
    #     wait(2000)
    #     gyro_sensor.reset_angle(0)
    #     wait(1000)
    ...
    #     gyroturn(speed=150, heading=180)
    #     rstop()
    # elif continuation==False and mountain[cell]==Color.BLUE and cell!=7:
    #     # robot.turn(180)
    #     to_wall(distance=200, speed=200, time_limit=1500)
    #     rstop()
    #     wait(2000)
    ...
    if mountain[2]==Color.PURPLE and mountain[3]==Color.BROWN:
        robot.straight(-40)
        hand_motor.run_until_stalled(1500,Stop.HOLD,50)
    elif mountain[2]==Color.BLUE and mountain[3]==Color.BROWN:
        robot.straight(-57)
        hand_motor.run_until_stalled(1500,Stop.HOLD,50)
    elif mountain[2]==Color.RED and mountain[3]==Color.BROWN:
        robot.straight(-30)
        hand_motor.run_until_stalled(1500,Stop.HOLD,50)
    elif mountain[2]==Color.GREEN and mountain[3]==Color.BROWN:
        robot.straight(-20)
        hand_motor.run_until_stalled(1500,Stop.HOLD,50)
    # if continuation==False and mountain[cell]==Color.GREEN and cell!=7:
    # if continuation==False and mountain[cell]==Color.BLUE and cell!=7:
    # if continuation==False and mountain[cell]==Color.PURPLE and cell!=7:


# hand_motor.run_until_stalled(1500,Stop.BRAKE,50)
# hand_motor.run_target(1500,130)

# pid_left_to_rightREDX()
# gyroturn(speed=100, heading=180)
# rstop()
# gyropid_to_motor(distance=2000,speed=100)
# rstop()
...
hand_motor.run_until_stalled(1500,Stop.HOLD,50)
mountain_scan(0)
collection_of_gems(cell=0,Blue=True,Red=True,continuation=True,blind=False)
mountain_scan(1)
collection_of_gems(cell=1,Blue=True,Red=False,continuation=True,blind=False)
mountain_scan(2)
collection_of_gems(cell=2,Blue=True,Red=True,continuation=True,blind=False)
mountain_scan(3)
collection_of_gems(cell=3,Blue=False,Red=False,continuation=False,blind=True)

to_wall(distance=350, speed=200, time_limit=2500)
rstop()
wait(2000)
gyro_sensor.reset_angle(0)
wait(1000)
...
rstop()
# robot.straight(150)
gyropid_to_motor(speed=100,p=0.3,i=0.001,d=10,distance=130,heading=180)
gyroturn(speed=100, heading=90)
rstop()
Dam_opening()
...
# mountain_scan(4)
# collection_of_gems(cell=4,Blue=True,continuation=True)
# mountain_scan(5)
# collection_of_gems(cell=5,Blue=True,continuation=True)
# mountain_scan(6)
# collection_of_gems(cell=6,Blue=True,continuation=True)
# mountain_scan(7)
# collection_of_gems(cell=7,Blue=False,continuation=False)
...
# robot.reset()
# robot.drive(600,140)
# while robot.angle()<160: pass # 171
# rstop()
